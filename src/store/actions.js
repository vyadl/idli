export default {
  ...
  async _fetchListsForUser({ commit, dispatch }) {
    const { data: responseLists } = await this.$conf.axios.get(`${this.$conf.apiBasePath}lists`);

    commit('setLists', responseLists);
    dispatch('_setListIdFromLocalStorage');
  },
  async _fetchTestLists({ commit }) {
    const { data: responseLists } = await this.$conf.axios.get('/test_data.json');
  ...
  async _fetchListById({ commit, dispatch, getters }, { id, cancelToken }) {
    ...
    const { data: responseList } = await this.$conf.axios
      .get(`${this.$conf.apiBasePath}list/${id}`, { cancelToken });
  ...
  async _fetchCurrentItems({ commit, getters }) {
    const { data: responseList } = await this.$conf.axios
      .get(`${this.$conf.apiBasePath}list/${getters.currentListId}`);

    commit('setCurrentItems', responseList.items);
  },
  async _addList({ commit, dispatch }, list) {
    const { data: responseList } = await this.$conf.axios
      .post(`${this.$conf.apiBasePath}list/add`, list);
  ...
  async _addTestList({ commit, dispatch }, {
    ...
    const { data: responseList } = await this.$conf.axios
      .post(`${this.$conf.apiBasePath}list/add`, {
        title,
        isPrivate,
        tags,
        categories,
    ...
    const { data: responseItems } = await this.$conf.axios
      .post(`${this.$conf.apiBasePath}items/add-many/${responseList.id}`, { items });
  ...
  async _updateList({ commit }, {
    ...
    const { data: responseList } = await this.$conf.axios
      .patch(`${this.$conf.apiBasePath}list/update/${id}`, {
        title,
        isPrivate,
        tags,
        categories,
  ...
  async _deleteList({ commit, dispatch, getters }, id) {
    await this.$conf.axios.delete(`${this.$conf.apiBasePath}list/delete/${id}`);
  ...
  async _addItem({ commit, getters }, item) {
    ...
    const { data: responseItem } = await this.$conf.axios
      .post(`${this.$conf.apiBasePath}item/add/${listId}`, item);
  ...
  async _updateItem({ commit, dispatch }, {
    ...
    const { data: responseItem } = await this.$conf.axios
      .patch(`${this.$conf.apiBasePath}item/update/${listId}/${id}`, {
        title,
        details,
        tags,
        category,
  ...
  async _deleteItem({ commit, dispatch }, item) {
    await this.$conf.axios.delete(`${this.$conf.apiBasePath}item/delete/${item.listId}/${item.id}`);
  ...
  async _fetchDeletedLists({ commit }) {
    const { data: deletedLists } = await this.$conf.axios.get(`${this.$conf.apiBasePath}lists/deleted`);

    commit('setDeletedLists', deletedLists);
  },

  async _fetchDeletedItems({ commit }) {
    const { data: deletedItems } = await this.$conf.axios.get(`${this.$conf.apiBasePath}items/deleted`);

    commit('setDeletedItems', deletedItems);
  },

  async _restoreList({ dispatch, getters }, listId) {
    const res = await this.$conf.axios.patch(`${this.$conf.apiBasePath}list/restore/${listId}`);
  ...
  async _restoreItem({ dispatch }, { listId, itemId }) {
    const res = await this.$conf.axios.patch(`${this.$conf.apiBasePath}item/restore/${listId}/${itemId}`);
  ...
  async _hardDeleteList({ dispatch }, listId) {
    await this.$conf.axios.delete(`${this.$conf.apiBasePath}list/hard-delete/${listId}`);

    dispatch('_fetchDeletedLists');
  },

  async _hardDeleteItem({ dispatch }, { listId, itemId }) {
    await this.$conf.axios.delete(`${this.$conf.apiBasePath}item/hard-delete/${listId}/${itemId}`);

    dispatch('_fetchDeletedItems');
  },

  async _hardDeleteAllItems({ dispatch }) {
    await this.$conf.axios.delete(`${this.$conf.apiBasePath}item/hard-delete-all`);

    dispatch('_fetchDeletedItems');
  },

  async _restoreAllItems({ dispatch }) {
    const res = await this.$conf.axios.patch(`${this.$conf.apiBasePath}item/restore-all`);
  ...
  async _hardDeleteAllLists({ dispatch }) {
    await this.$conf.axios.delete(`${this.$conf.apiBasePath}list/hard-delete-all`);

    dispatch('_fetchDeletedLists');
  },

  async _restoreAllLists({ dispatch, getters }) {
    const res = await this.$conf.axios.patch(`${this.$conf.apiBasePath}list/restore-all`);
